...
export default async function UFMPolicyPage() {
  ...
  try {
    ...
    dbPolicies = policiesRaw.map((p) => ({
      ...
      file_link: p.pdfUrl || p.file_link || "",
      ...
      date: p.updatedAt ? new Date(p.updatedAt).toISOString() : (p.date ? new Date(p.date).toISOString() : new Date().toISOString()),
      updatedDate: p.updatedAt ? `Updated ${new Date(p.updatedAt).toLocaleDateString("en-US", { month: "short", day: "2-digit", year: "numeric" })}` : (p.date ? `Updated ${new Date(p.date).toLocaleDateString("en-US", { month: "short", day: "2-digit", year: "numeric" })}` : "Updated recently"),
